...
import { listCollaborations } from '../services/userService';
...
export default function MyCollabsScreen() {
  ...
  useEffect(() => {
    const fetchCollabs = async () => {
      if (!token) return;
      try {
        const res = await listCollaborations(token, { status: 'ACCEPTED', direction: 'all' });
        if (res.success) {
              ...
              return (
                ...
                >
                  ...
                  <View className="flex-1 justify-center">
                    <Text className="text-white text-[15px] font-semibold" style={{ fontFamily: 'Poppins_600SemiBold' }}>{otherName}</Text>
                  ...
                  <View className="rounded-full border px-2.5 py-[5px] bg-[#10B981]/12 border-[#10B981]/30">
